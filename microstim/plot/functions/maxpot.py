...
ax.hlines(20, 0 , max(X_RANGE))
ax.spines['right'].set_visible(False)

# Use the Seaborn palette colors
plt.plot(X_RANGE, np.max(v_e, axis=0), color=palette[1], label=r"amp $v_e$")
plt.plot(X_RANGE, np.max(v_i, axis=0), color=palette[2], label=r"amp $v_i$")
plt.plot(X_RANGE, np.max(no_amp, axis=0), color=palette[0], label="no amp")
