...
linspace = np.arange(rheobase + 0.01, rheobase*5, 0.01)  # Start slightly above rheobase
...
for point in linspace:
    _, _, ratio_e, ratio_i = axon(linspace, rheobase, pulse, point)
    try:
...
_, _, ratio_e, ratio_i = axon(linspace, rheobase, pulse, intensity)
try:
...
plt.plot(linspace, ratios, color=palette[1], label="Ratio Distribution")
