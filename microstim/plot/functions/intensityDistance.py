...
for i in intensity:
    print("intensity %i" % i)
    _, _, rho_e, rho_i, _, _ = model(i, weights, sigma, rect, boost, is_depolarized=True)
    _, _, no_rho, _, _, _ = model(i, no_boost_weights, sigma, rect, no_boost, is_depolarized=True)
...
plt.legend(loc="best")  
plt.savefig("results/amp2/svg/intensityRadius.svg", format="svg", bbox_inches="tight")
plt.savefig("results/amp2/intensityRadius.png", format="png", bbox_inches="tight")
